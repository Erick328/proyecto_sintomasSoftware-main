...
class _CancerPageState extends State<CancerPage> {
  ...
  final primerMensaje = 'Hola, ¿en que puedo ayudarte?';
          ...
          if(this.speech.isNotListening){
            if (this.mensajeControlador.text.trim().length > 0){
              ...
              if (text == 'Cancer'){
                ...
                // this._enviarMensaje('$text ${this.mensajeControlador.text}', text);
              }
            }
          }
        });
  ...
  void _enviarMensaje(String texto, text) async{
    ...
    String textoNuevo = 'Cancer ' + texto;
    print(textoNuevo);
    var textResp = await this.dialogFlowService.sendMessage(textoNuevo, this.user.id);
